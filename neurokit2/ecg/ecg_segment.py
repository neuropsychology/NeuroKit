...
def ecg_segment(ecg_cleaned, rpeaks=None, sampling_rate=1000, show=False):
    ...
    >>> fig = nk.ecg_segment(ecg_cleaned, rpeaks=None, sampling_rate=1000, show=True)
    >>> fig #doctest: +SKIP
    """
    # Sanitize inputs
    if rpeaks is None:
    ...
    if show:
        ...
        heartbeats_pivoted = heartbeats_plot.pivot(index='Time',
                                                   columns='Label',
                                                   values='Signal')
        ...
        cmap = iter(plt.cm.YlOrRd(
                np.linspace(0, 1,
                            num=int(heartbeats_plot["Label"].nunique()))))
        lines = []
