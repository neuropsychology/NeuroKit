...
def ecg_find_peaks(ecg, sampling_rate, **kwargs):
    """
    """
...
def main():
    ...
    ecg_clean = my_cleaning(ecg, sampling_rate=sampling_rate, smooth=True)
    # first derivative of the Gaissian signal
    scales = np.array([1, 2, 4, 8, 16, 32])
    ...
    if show_significant_peaks:
        nk.events_plot(np.concatenate(significant_peaks_groups), ecg_clean)
    ...
    # nk.events_plot(tpeaks, ecg)
    # nk.events_plot(ppeaks, ecg)


    # remove overlap peaks?
    ...
    p_half_width = int(0.2*sampling_rate)  # NEED TO CHECK
    p_onsets = []
    p_offsets = []
    left_base=[]
    for index_p_peak in ppeaks:
        ...
        prominence = 0.0*max(search_window)
        height = 0.0
        peaks, peaks_data = find_peaks(search_window, height=height, prominence=prominence)
        # The last peak is nfirst in (Martinez, 2004)
        # nk.events_plot(peaks, search_window); plt.show()
