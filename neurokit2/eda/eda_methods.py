...
def eda_methods(
    ...
    **kwargs
    ...
    method_phasic = (
        str(method).lower()
        if method_phasic == "default"
        else str(method_phasic).lower()
    )
    method_peaks = (
    ...
    if method_cleaning == "biosppy":
        ...
        report_info[
            "text_cleaning"
        ] += " was cleaned using the default method of the neurokit2 package."
    elif method_cleaning in ["none"]:
        report_info["text_cleaning"] += "was directly used without cleaning."
    else:
        report_info["text_cleaning"] += (
            " was cleaned using the method described in " + method_cleaning + "."
        )

    # 2. Phasic decomposition
    # -----------------------
    # TODO: add descriptions of individual methods
    report_info["text_phasic"] = (
        "The signal was decomposed into phasic and tonic components using"
    )
