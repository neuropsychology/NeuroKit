...
def rsp_analyze(data, sampling_rate=1000, method="auto"):
    ...
    data : dict, DataFrame
        ...
        a longer period of data, typically generated by `rsp_process()`
    ...
    -------
    DataFrame
        A dataframe containing the analyzed RSP features. If
        event-related analysis is conducted, each epoch is indicated
        by the `Label` column. See `rsp_eventrelated()` and
        `rsp_intervalrelated()` docstrings for details.
