...
def rsp_plot(rsp_signals, sampling_rate=None):
    ...
    ax0.plot(x_axis, rsp_signals["RSP_Raw"], color='lightblue', label='Raw', zorder=1)
    ax0.plot(x_axis, rsp_signals["RSP_Clean"], color='darkblue', label='Cleaned', zorder=1)
    ...
    ax1.plot(x_axis, rsp_signals["RSP_Rate"], color='blue', label='Data')
    rate_mean = [np.mean(rsp_signals["RSP_Rate"])]*len(rsp_signals["RSP_Rate"])
    ax1.plot(x_axis, rate_mean, label='Mean', linestyle='--', color='lightblue')
    ax1.set_ylabel('Inhales per minute')
    ax1.legend(loc='upper right')
    ax2.set_title("Breathing Amplitude")
    ax2.plot(x_axis, rsp_signals["RSP_Amplitude"], color='green', label='Data')
    amplitude_mean = [np.mean(rsp_signals["RSP_Amplitude"])]*len(rsp_signals["RSP_Amplitude"])
    ax2.plot(x_axis, amplitude_mean, label='Mean', linestyle='--', color='lightgreen')
    ax2.set_ylabel('a.u.')
    if sampling_rate is not None:
