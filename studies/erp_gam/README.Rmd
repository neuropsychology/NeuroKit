...
model <- mgcv::gam(EEG ~ Condition + s(Time, by = Condition, bs="cr"), data=data, method="REML")
```
...
model <- mgcv::gam(EEG ~ Condition + s(Time, by = Condition, k = 0.05 * 600, bs="cr"), data=data, method="REML")
```
...
# model <- gamm4::gamm4(EEG ~ Condition + s(Time, by = Condition, k = 0.05 * 600, bs="cr"),
#                       random=~(1|Channel), data=data_long)
