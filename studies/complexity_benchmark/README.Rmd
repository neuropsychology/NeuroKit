...
# plot(rez)
closest <- attributes(rez)$closest_component
see::material_colors()
...
loadings |> 
  ...
  theme(axis.text.y = element_text(
    color = rev(rev(see::palette_material("rainbow")(max(closest)))[closest]),
    face = ifelse(rez$Variable %in% c("SD", "Length", "Noise"), "italic", "plain")
),
        plot.title = element_text(hjust = 0.5),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) 
...
#   ggplot(aes(x = Duration, y = Index)) +
#   geom_bar(stat = "identity") +
#   scale_x_log10(breaks = rep(10, 5)**seq(0, 4), labels = function(x) sprintf("%g", x)) +
#   scale_fill_manual(values = colors, guide = "none") +
#   labs(x = NULL, y = "Computation Time")
```
