...
rez <- parameters::factor_analysis(data, cor = r, n = 11, rotation = "varimax", sort = TRUE)
# rez <- parameters::principal_components(data, n = 5, sort = TRUE)
rez

closest <- colnames(select(rez, starts_with("PC")))[apply(select(rez, starts_with("PC")), 1, \(x) which.max(abs(x)))]

loadings <- attributes(rez)$loadings_long |> 
  mutate(Loading = abs(Loading),
         Component = fct_relevel(Component, rev(names(select(rez, starts_with("PC"))))),
