...
As complexity science grows in size and application, a systematic approach to compare their "performance" becomes necessary to increase the clarity and structure of the field. The word *performance* is to be understood in a relative sense, as any such endaveour faces the "hard problem" of complexity science. The indices are sensitive to specific objective properties of a signal, features we consider part of over-arching concepts such as "complexity" and "chaotic". But it is unclear how these concepts transfer in a top-down fashion into a combination of lower-level features, such as short-term vs. long-term variability, auto-correlation, information, randomness, and so on. As such, it is conceptually complicated to benchmark complexity measures against a "objectively" complex vs. non-complex signals. In other words, we know that different characteristics can contribute to the "complexity" of a signal, but there is not a one-to-one correspondence between the latter and the former. 

Hence the choice of the current paradigm

and some indices might be more or less sensitive to these features  
