...
g <- cor |> 
  cor_lower() |> 
  mutate(width = abs(rho),
         edgecolor = as.character(sign(rho))) |> 
  tidygraph::as_tbl_graph(directed=FALSE) 

g |> 
  mutate(importance = tidygraph::centrality_hub(weights = abs(rho)),
         group = tidygraph::group_fast_greedy(weights = abs(rho))) |> 
  ...
  ggraph::geom_node_point(aes(size = importance, fill = group)) +
