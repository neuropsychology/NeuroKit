...
```{r computation_time, message=FALSE, warning=FALSE, fig.width=16*1.25, fig.height=10*1.25, cache=FALSE}
order <- df |>
  group_by(Index) |>
...
```{r time2, message=FALSE, warning=FALSE, include=FALSE}
df |>
  filter(!Index %in% c("SD", "Length", "Noise", "Random")) |>
  mutate(Duration = Duration * 10000) |>
...
```{r eval=TRUE, message=FALSE, warning=FALSE, cache=FALSE, include=FALSE}
cor |>
  cor_lower() |>
...
g |>
  mutate(importance = tidygraph::centrality_authority(weights = abs(r)),
         group = as.factor(tidygraph::group_fast_greedy(weights = abs(r)))) |>
  ...
  scale_edge_width_continuous(range = c(0.01, 2), guide = "none") +
...
rez <- parameters::factor_analysis(data, cor = r, n = 15, rotation = "geominT", sort = TRUE, fm="mle")
