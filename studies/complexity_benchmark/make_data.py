...
def run_methods(i, x, delay=1, dimension=2):
    ...
    # data.append(
    #     time_function(
    #         x, nk.fisher_information, index="Fisher", name="nk_fisher_information"
    #     )
    # )
    # data.append(
    #     time_function(x, nk.entropy_shannon, index="ShanEn", name="nk_entropy_shannon")
    # )
    # data.append(
    #     time_function(
    #         x,
    ...
    # data.append(
    #     time_function(
    #         x, nk.entropy_differential, index="DiffEn", name="nk_entropy_differential"
    #     )
    # )
    # data.append(time_function(i, x, nk.entropy_svd, index="SVDen", name="nk_entropy_svd"))
    # data.append(
    #     time_function(x, nk.entropy_spectral, index="SpEn", name="nk_entropy_spectral")
    # )
    # data.append(time_function(x, nk.fractal_katz, index="Katz", name="nk_fractal_katz"))
    # data.append(
    #     time_function(x, nk.fractal_sevcik, index="Sevcik", name="nk_fractal_sevcik")
    # )
    # data.append(
    #     time_function(
    #         x, nk.fractal_petrosian, index="FD_Petrosian", name="nk_fractal_petrosian"
    #     )
    # )
    data = pd.concat(data)
    data["Iteration"] = i
...
# Iterations
data = []
for n in nk.expspace(100, 10 ** 3, 10).astype(int):
    print(n)
    ...
    args = [{"x": x, "i": i, "dimension": dimension, "delay": delay} for i in range(100)]
    out = nk.parallel_run(
        run_methods,
        args,
        n_jobs=-3,
