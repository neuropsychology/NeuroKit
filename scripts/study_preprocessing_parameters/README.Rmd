...
df <- read.csv("data_RSP_filtering.csv") %>% 
  mutate(Filter_Bandpass = paste0(insight::format_value(Filter_Low), ", ", insight::format_value(Filter_High)))
...
<p>
```{r, echo = TRUE, warning=FALSE, message=FALSE}
data <- select(df, Difference_Cleaned_Mean, Filter_Type, Filter_Bandpass)
model <- lm(Difference_Cleaned_Mean ~ Filter_Type * Filter_Bandpass, data=data[complete.cases(data),])
...
fig <- model %>% 
  estimate_means() %>% 
  ggplot(aes(x = Filter_Type, y = Mean, color=Filter_Bandpass)) +
  geom_line(aes(group=Filter_Bandpass), position = position_dodge(width = 0.2)) +
