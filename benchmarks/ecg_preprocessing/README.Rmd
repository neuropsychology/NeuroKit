...
The script to download and format the database using the [**ECG-GUDB**](https://github.com/berndporr/ECG-GUDB) Python package by Bernd Porr can be found [**here**](TODO).
...
data %>% 
  ...
  ggplot(aes(x=Method, y=Duration, fill=Method)) +
    geom_boxplot(aes(color=Database), outlier.alpha = 0, alpha=1) +
