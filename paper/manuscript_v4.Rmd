...
It is an open-source Python package, developed by a multi-disciplinary team that actively invites new collaborators. The target audience of *NeuroKit2* includes both experienced and novice programmers, where documentation (*https://neurokit2.readthedocs.io/en/latest/start/learn_python.html*) on getting started with Python is available for the latter. While many of the existing software caters to only one specific signal type
such as *KUBIOS* [@tarvainen2014kubios], *HeartPy* [@HeartPy] and *pyHRV* [@gomes2019pyhrv] for ECG signal, *cvxEDA* [@greco2015cvxeda], *Ledalab* [@benedek2010continuous], and *SCRalyze* [@bach2014head] for EDA signal, *NeuroKit2* provides support for various signals and allows its users to process multiple types of signals at once, using the high-level meta-functions (more details in the **Design Philosophy**). It aims at being accessible, well-documented, well-tested, cutting-edge, flexible and efficient, allowing users to select from a wide range of validated analysis pipelines as well as creating their own.
...
*NeuroKit2* offers a breadth of functionalities which includes, but is not limited to: (1) signal simulation, (2) data management (e.g., downloading data from the GitHub repository, reading and formatting files into a dataframe), (3) events extraction from signals, (4) epochs extraction, (5) signal processing (e.g., filtering, resampling, rate computation) (6) spectral analyses such as computing power spectral densities, (7) complexity analyses such as time-delay embedding of time series data, (8) entropy computation, (9) clustering analysis using a variety of different algorithms (e.g., K-means, independent component analysis, principal component analysis) and (10) statistical analysis such as polynomial regressions and kernel density estimations. There is also an extensive host of plotting options, allowing for quick visualization of processed signals (e.g., raw, cleaned, and decomposed signals) as well as visualization of events in a signal. *NeuroKit2* can also perform event-related analysis and resting-state analysis on specific bodily signals (as demonstrated in the *Examples* section of this paper) and EEG microstates segmentation, a functionality currently being expanded and improved on.


<!-- Accessibility and documentation -->
The package is implemented in Python 3 [@python3], which means that *NeuroKit2's* users benefit from an large number of learning resources and a vibrant community. The package depends on relatively few, well established and robust packages from the Python data analysis ecosystem [@scipy] such as *NumPy* [@numpy], *pandas* [@pandas], *SciPy* [@scipy], *scikit-learn* [@scikit-learn] and *MatplotLib* [@matplotlib] (with an additional system of optional dependencies), making *NeuroKit2* itself a viable dependency in other software.

*NeuroKit2's* source code is available under the permissive MIT license on GitHub (*https://github.com/neuropsychology/NeuroKit*). Its documentation (*https://neurokit2.readthedocs.io/*) is automatically built and rendered from the code and includes guides for installation and contribution, a description of the package's functions, as well as several "hands-on" examples and tutorials (e.g., how to extract and visualize individual heartbeats, how to analyze event-related data etc.). Importantly, users can add new examples by simply submitting a Jupyter notebook [@kluyver2016jupyter] to the GitHub repository. The notebook will automatically be displayed on the website, ensuring easily accessible and evolving documentation. Moreover, users can try out the example notebooks directly in their browser via a cloud-based *Binder* environment [@Jupyter2018]. Finally, the issue tracker on GitHub offers a convenient and public forum that allows newcomers and potential collaborators to report bugs, get help and gain insight into the development of the package. Our active collaborators range from academics from bioinformatics, biomedical sciences and physics, as well as robotics and electrical engineers (See *https://neurokit2.readthedocs.io/en/latest/authors.html* for full list of active collaborators). Based on the discussion platforms on GitHub, *NeuroKit2* has attracted users of different profiles, ranging from those who are new to signal processing and programming to more experienced users. As of 17 September 2020, *NeuroKit2* has been downloaded **630 times** per month (excluding the direct download from GitHub).


<!-- Reliability and Evolution -->
*NeuroKit2* aims at being reliable and trustworthy, including processing pipelines that have been described in peer-reviewed literature and functions that have been tested against established software such as *BioSPPy* [@biosppy], *hrv* [*under review*](https://github.com/openjournals/joss-reviews/issues/1867), *PySiology* [@PySiology], *HeartPy* [@HeartPy], *systole* [@Systole] or *nolds* [@nolds]. Unittest-based tests are implemented through *pytest* which provides comprehensive reports of test results, making code failures easibly traceable. As of 25 August 2020, *NeuroKit2* has a code coverage of 83.3% and its build status can be tracked via its GitHub repository. Overall, the repository leverages a comprehensive test suite and continuous integration (CI) through the Travis-CI platform and GitHub actions to ensure stability and prevent errors. Thanks to its collaborative and open development, *NeuroKit2* can remain cutting-edge and continuously evolve, adapt, and integrate new methods as they are emerging.
...
*NeuroKit2* can be installed using pip as `pip` as demonstrated in this short tutorial here (*https://neurokit2.readthedocs.io/en/latest/installation.html#neurokit*). This `readthedocs` page also comprises of a full list of instructions on how to install Python (supported versions are 3.7, 3.8 and 3.9) with different operating systems.
...
It is also a pragmatic answer to the broader need for transparent and reproducible methods in neurophysiology. Its impact on reproducibility in research is two-fold: firstly, while black-box softwares are easy and convenient to use, users do not have access to the source code. The final results often depend on the software used and each software differs in the methods used in their analysis pipeline. This makes it difficult to identify the source of discrepancy in unreplicable findings. In contrast, *NeuroKit2* documents each step of the implementation along with the analysis method used, allowing users to pin-point the analysis steps where differences in results arise. While maintaining a focus on overall user-experience, the open-source nature of *NeuroKit2* encourages independent researchers to cross-validate research findings. Secondly, not only does *NeuroKit2* implement several methods for analysis, it also compares the performance of different algorithms. For instance, using a suite of open-source databases, different algorithms for ECG R-peak detection have been compared for its robustness (number of errors encountered), efficiency (computation time) and accuracy (absolute distance from true R-peak location). This is documented on the GitHub repository (*https://github.com/neuropsychology/NeuroKit/tree/master/studies/ecg_preprocessing#study-1-comparing-different-r-peaks-detection-algorithms*). As *NeuroKit2* continues to work on benchmarking, future differences observed between methods can be better explained. Moreover, this guides users to make more informed decisions regarding which method is most suited for their data analysis.

*NeuroKit2* also prioritizes a high standard of quality control during code development. This is done through automated testing of code using unittest-based tests that follow coding best practices, reducing code complexity and maintaining code readability. Additionally, we provide guidelines for new contributors to follow when writing  code, encouraging them to follow PEP 8 coding conventions [@pep8] and fix code errors, as documented in our `readthedocs` page (*https://neurokit2.readthedocs.io/en/latest/contributing/contributing.html*). Automation through GitHub actions also continually builds and updates the documentation of 
*NeuroKit2* functions. We have a fairly complete API documentation with each function thoroughly documented, including working examples. While *NeuroKit2* currently has a fairly comprehensive documentation, more examples and tutorials will be added as the package grows and expands.
