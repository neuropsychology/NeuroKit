...
In this section, we present two examples that illustrate the most common use-cases. The first example represents an event-related paradigm, in which the interest lies in short-term physiological changes related to specific events (see **Figure 2** and **Table 1**). The second example shows how to extract the characteristics of physiological activity during a longer period of time (not necessarily tied to a specific and sudden event). The example datasets are available with the package and can be downloaded using the `data()` function.
