...
Besides the signal-specific modules, there are modules with low-level functions, such as signal, complexity, misc and stats, that support the implementation of core functionality in other modules. Unless the users wish to, they do not need to call these low-level functions into the processing scripts. 

Last but not least, there are modules with functions that also perform specific steps in the processing pipeline but are non-specific to any type of signal. Such as *events* to support incorporation and visualization of event markers in the signal or *epochs* to chunk signals into event-related segments.  
