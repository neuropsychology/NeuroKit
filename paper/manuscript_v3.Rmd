...
abstract: |
  NeuroKit2 is an open-source, community-driven, and user-friendly Python package dedicated to neurophysiological signal processing (e.g., ECG, EDA, EMG, EOG, PPG etc.). Its design philosophy is centred on user-experience and accessibility to both novice and advanced users. The package provides a consistent set of high-level functions that enable data processing in a few lines of code using validated pipelines, which we illustrate in two data analysis examples in this paper: (1) Event-related Analysis and (2) Interval-related Analysis. At the same time, it also comprises of tools dedicated to specific processing steps such as signal rate extraction and signal filtering methods, offering a trade-off between efficiency and fine-tuned control. As most openly available software libraries focus on processing signal-specific data, NeuroKit2 was developed to provide a comprehensive means for preprocessing and analyzing different signals. It aims at improving transparency and reproducibility in neurophysiological research, as well as fostering exploration and innovation.
