...
def test_signal_surrogate():
    ...
    # Check distribution
    assert np.allclose(np.histogram(x, 10, (0, 1))[0],
                       np.histogram(x, 10, (0, 1))[0], atol=1)
    # Check spectrum
    assert np.mean(np.abs(np.abs(np.fft.rfft(surrogate - np.mean(surrogate))) -
                          np.abs(np.fft.rfft(x - np.mean(x))))) < .1
