...
def test_read_acqknowledge():
    ...
    df, sampling_rate = nk.read_acqknowledge(
        os.path.join(path_data, "acqnowledge.acq"), sampling_rate=2000
    )
    assert sampling_rate == 2000

    df, sampling_rate = nk.read_acqknowledge(
        os.path.join(path_data, "acqnowledge.acq"), sampling_rate="max"
    )
...
def test_data():
    ...
    assert all(
        elem in ["ECG", "EDA", "Photosensor", "RSP"]
    ...
    assert all(
        elem in np.array(data.columns.values, dtype=str)
