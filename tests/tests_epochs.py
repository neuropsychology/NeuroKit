...
def test_epochs_create():
    ...
    # Create epochs
    epochs_1 = nk.epochs_create(
        data, events, sampling_rate=100, epochs_start=-0.5, epochs_end=3
    )
    epochs_2 = nk.epochs_create(
        data, events, sampling_rate=100, epochs_start=-0.5, epochs_end=1
    )
    ...
    assert all(
        elem in columns for elem in np.array(epochs_1["1"].columns.values, dtype=str)
    )
